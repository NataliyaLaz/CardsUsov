...
extension ShapeLayerProtocol {
    init(){
        fatalError("init() can't be used for object creation")
    }
}
...
class SquareShape: CAShapeLayer, ShapeLayerProtocol{
    required init(size: CGSize, fillColor: CGColor) {
        ...
        let squareSide = [size.width, size.height].min() ?? 0
        let rect = CGRect(x: 0, y: 0, width: squareSide, height: squareSide)
...
extension UIResponder {
    func responderChain() -> String {
        guard let next = next else { return String(describing: Self.self) }
        return String(describing: Self.self) + "->" + String(describing: next.responderChain())
    }
}
